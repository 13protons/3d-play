 ...
 * (drives the limiting-magnitude fade in MagnitudeStars) and blackbody `color`.
 */
...
export function loadStarCatalog(): Promise<StarCatalog> {
  if (!cached) {
    cached = fetch(ASSET_URL)
      ...
      .catch((err) => {
        // Don't leave a rejected promise cached — clear it so a later mount can retry.
        cached = undefined
        throw err
      })
  }
...
export function buildStarInstanceData(catalog: StarCatalog, radius: number): StarInstanceData {
  const { count, directions, magnitudes, colors } = catalog
  ...
  return { count, positions, magnitudes: magnitudes.slice(), colors: colorFloats }
}
