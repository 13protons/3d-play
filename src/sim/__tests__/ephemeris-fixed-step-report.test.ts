...
describe('fixed-step ephemeris report', () => {
  it('prints one-year drift with exactly one fixed step per minute', () => {
    expect(ONE_YEAR_MINUTE_STEPS).toBe(525_600)
    ...
    advanceVelocityVerlet(state, gms, ONE_MINUTE_SECONDS, ONE_YEAR_MINUTE_STEPS)

    const references = new Map(JPL_FULL_SOLAR_SYSTEM_ONE_YEAR.map((body) => [body.id, body.position]))
    console.log(`fixed steps: ${ONE_YEAR_MINUTE_STEPS}`)
